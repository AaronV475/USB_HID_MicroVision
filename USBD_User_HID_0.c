...
bool USBD_HID0_SetReport (uint8_t rtype, uint8_t req, uint8_t rid, const uint8_t *buf, int32_t len) {
  ...
  switch (rtype) {
    case HID_REPORT_OUTPUT:
      if (*buf == 0x00) { HAL_GPIO_TogglePin(LED1_GPIO_Port, LED1_Pin); }
			else if (*buf == 0x01) { HAL_GPIO_TogglePin(LED2_GPIO_Port, LED2_Pin); }
			else if (*buf == 0x02) { HAL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin); }
			else if (*buf == 0x03) { HAL_GPIO_TogglePin(LED4_GPIO_Port, LED4_Pin); }
			else if (*buf == 0x04) { HAL_GPIO_TogglePin(LED5_GPIO_Port, LED5_Pin); }
			else if (*buf == 0x05) { HAL_GPIO_TogglePin(LED6_GPIO_Port, LED6_Pin); }
			else if (*buf == 0x06) { HAL_GPIO_TogglePin(LED7_GPIO_Port, LED7_Pin); }
			else if (*buf == 0x07) { HAL_GPIO_TogglePin(LED8_GPIO_Port, LED8_Pin); }
      break;
